...
from nng.classification.ops import fisher_factors
from nng.classification.ops import utils
...
class FisherBlock(object):
    ...
    def make_scale_basis_update_ops(self):
        """ Update ops to update Eigen-basis registered in ScaleFactor.
        :return: Ops for updating Eigen-basis registered in ScaleFactor.
        """
        return []
...
class EigenCorrectedKroneckerProductFB(FisherBlock):
    """A core class for FisherBlocks with separate input and output factors.
    The Fisher block is approximated as a Kronecker product of the input and
    output factors.
    """
    def _register_input_and_output_eigen_basis(self, damping):
        """ Initialize the input and output Eigen basis with the damping term.
        :param damping: float
        :return: None
        """
        self._input_damping, self._output_damping = _compute_pi_adjusted_damping(
            ...
            self._output_factor.get_cov(),
            damping ** 0.5)
        self._input_factor.register_eigen_basis(self._input_damping)
        self._output_factor.register_eigen_basis(self._output_damping)
    ...
    def full_fisher_block(self):
        ...
        left_basis = self._input_factor.get_eigen_basis(self._input_damping)
        right_basis = self._output_factor.get_eigen_basis(self._output_damping)
        scale_factor = self._scale_factor.get_cov()

        kron_product = utils.kronecker_product(left_basis, right_basis)
        kron_product_t = array_ops.transpose(kron_product)
        output = math_ops.matmul(math_ops.matmul(kron_product,
                                                 scale_factor),
                                 kron_product_t)

        return self._renorm_coeff * output


class EigenCorrectedFullyConnectedKFACBasicFB(EigenCorrectedKroneckerProductFB):
    """K-FAC FisherBlock for fully-connected (dense) layers.
    This uses the Kronecker-factorized approximation from the original
    K-FAC paper (https://arxiv.org/abs/1503.05671)
    """
    def __init__(self, layer_collection, has_bias=False):
        """Creates a EigenCorrectedFullyConnectedKFACBasicFB block.
        Args:
          layer_collection: The collection of all layers in the K-FAC approximate
              Fisher information matrix to which this FisherBlock belongs.
          has_bias: Whether the component Kronecker factors have an additive bias.
        ...
        super(EigenCorrectedFullyConnectedKFACBasicFB, self).__init__(layer_collection)
    ...
    def instantiate_factors(self, grads_list, damping):
        ...
        self._damping = damping
        self._input_factor = self._layer_collection.make_or_get_factor(  #
            fisher_factors.FullyConnectedEigenBasisFactor,  #
            ((inputs,), self._has_bias))
        self._output_factor = self._layer_collection.make_or_get_factor(  #
            fisher_factors.FullyConnectedEigenBasisFactor,  #
            (grads_list,))

        # Initialize input and output Eigen Basis.
        self._register_input_and_output_eigen_basis(self._damping)
        self._scale_factor = self._layer_collection.make_or_get_scale_factor(  #
            fisher_factors.FullyConnectedScaleFactor,  #
            (inputs, grads_list, self._has_bias))
        # Initialize scale factor Eigen Basis.
        self._scale_factor.register_eigen_basis(
            self._input_factor.get_eigen_basis(self._input_damping),
            self._output_factor.get_eigen_basis(self._output_damping),
            self._input_factor.get_eigen_value(self._input_damping),
            self._output_factor.get_eigen_value(self._output_damping))
...
class EigenCorrectedConvKFCBasicFB(EigenCorrectedKroneckerProductFB):
    """FisherBlock for 2D convolutional layers using the basic KFC approx.
    Estimates the Fisher Information matrix's blog for a convolutional
    layer.
    Consider a convoluational layer in this model with (unshared) filter matrix
    ...
    """
    def __init__(self, layer_collection, params, strides, padding):
        """Creates a EigenCorrectedConvKFCBasicFB block.
        Args:
          layer_collection: The collection of all layers in the K-FAC approximate
              Fisher information matrix to which this FisherBlock belongs.
          params: The parameters (Tensor or tuple of Tensors) of this layer. If
    ...
    def instantiate_factors(self, grads_list, damping):
        ...
        self._num_locations = _num_conv_locations(inputs.shape.as_list(),
                                                  self._strides)
        self._input_factor = self._layer_collection.make_or_get_factor(
            fisher_factors.ConvInputEigenBasisKroneckerFactor,
            (inputs, self._filter_shape, self._strides, self._padding,
             self._has_bias))
        self._output_factor = self._layer_collection.make_or_get_factor(
            fisher_factors.ConvOutputEigenBasisFactor, (grads_list,))

        # Infer number of locations upon which convolution is applied.
        if NORMALIZE_DAMPING_POWER:
            eigen_damping = damping / (self._num_locations ** NORMALIZE_DAMPING_POWER)
        else:
            raise NotImplementedError
        self._register_input_and_output_eigen_basis(eigen_damping)

        self._scale_factor = self._layer_collection.make_or_get_scale_factor(
            fisher_factors.ConvScaleFactor,
            (inputs, grads_list, self._filter_shape,
             self._strides, self._padding, self._has_bias))
        self._scale_factor.register_eigen_basis(
            self._input_factor.get_eigen_basis(self._input_damping),
            self._output_factor.get_eigen_basis(self._output_damping),
            self._input_factor.get_eigen_value(self._input_damping),
