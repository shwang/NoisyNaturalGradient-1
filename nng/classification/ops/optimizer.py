...
from nng.classification.ops import estimator as est


class NGOptimizer(gradient_descent.GradientDescentOptimizer):
    """
    Optimizer for Natural Gradient.
    """
    ...
    def __init__(self,
        ...
        momentum_type = str(momentum_type).lower()
        ...
        if opt_type not in legal_opt_type:
            raise ValueError("Unsupported optimization type {}. Must be one of {}."
                             .format(opt_type, legal_opt_type))
        ...
        self.eigen_basis_update_op = self._fisher_est.eigen_basis_update_op
        self.scale_update_op = self._fisher_est.scale_update_op
        self.inv_update_op = self._fisher_est.inv_update_op
        self.inv_update_dict = self._fisher_est.inv_updates_dict
        self.re_init_scale_op = self._fisher_est.re_init_scale_op
        self.re_init_kfac_scale_op = self._fisher_est.re_init_kfac_scale_op

        super(NGOptimizer, self).__init__(learning_rate, name=name)
    ...
    def minimize(self, *args, **kwargs):
        ...
        if set(kwargs["var_list"]) != set(self.variables):
            raise ValueError("var_list doesn't match with set of Fisher-estimating "
                             "variables.")
        return super(NGOptimizer, self).minimize(*args, **kwargs)
    ...
    def compute_gradients(self, *args, **kwargs):
        ...
        if set(var_list) != set(self.variables):
            raise ValueError("var_list doesn't match with set of Fisher-estimating "
                             "variables.")
        return super(NGOptimizer, self).compute_gradients(*args, **kwargs)
    ...
    def apply_gradients(self, grads_and_vars, *args, **kwargs):
        ...
        return super(NGOptimizer, self).apply_gradients(steps_and_vars,
                                                          *args, **kwargs)
