...
from nng.classification.ops import fisher_blocks as fb
from nng.classification.ops import loss_functions as lf
from nng.classification.ops.utils import ensure_sequence, LayerParametersDict
...
_FULLY_CONNECTED_APPROX_TO_BLOCK_TYPES = {
    ...
    APPROX_EIGEN_NAME: fb.EigenCorrectedFullyConnectedKFACBasicFB
}

_CONV2D_APPROX_TO_BLOCK_TYPES = {
    APPROX_KRONECKER_NAME: fb.ConvKFCBasicFB,
    APPROX_DIAGONAL_NAME: fb.ConvDiagonalFB,
    APPROX_EIGEN_NAME: fb.EigenCorrectedConvKFCBasicFB
}
...
class LayerCollection(object):
    def __init__(self,
        ...
        if mode == "ekfac":
            self._default_fully_connected_approximation = APPROX_EIGEN_NAME
            self._default_convolution_2d_approximation = APPROX_EIGEN_NAME
        elif mode == "kfac":
            ...
            self._default_fully_connected_approximation = APPROX_FULL_NAME
            self._default_convolution_2d_approximation = APPROX_FULL_NAME
        elif mode == "diag":
            self._default_fully_connected_approximation = APPROX_DIAGONAL_NAME
            self._default_convolution_2d_approximation = APPROX_DIAGONAL_NAME
        else:
    ...
    def get_scale_factors(self):
        return self.scale_factors.values()
    ...
    def make_or_get_scale_factor(self, cls, args):
        try:
            hash(args)
        except TypeError:
            raise TypeError(
        ...
        if key not in self.scale_factors:
            colo = self._colocate_cov_ops_with_inputs
            with variable_scope.variable_scope(self._var_scope):
                self.scale_factors[key] = cls(*args, colocate_cov_ops_with_inputs=colo)
        return self.scale_factors[key]
